        ...
        <Card className=" min-w-[250px] md:w-[380px] lg:w-[99%] bg-blue-900/20  hover:bg-blue-900/20 border border-white/20">
          ...
          <CardContent>
            {totalAmount && title === "Total balance" ? (
              <p
                className={`${(totalAmount ?? 0) > 0 ? "text-green-500 text-2xl font-bold" : "text-red-500  text-2xl font-bold"} font-bold`}
              >
                {totalAmount.toFixed(2)} €
              </p>
            ) : (
              <p
                className={`${(amount ?? 0) > 0 && title !== "Outcomes" ? "text-green-500  text-2xl font-bold" : "text-red-500  text-2xl font-bold"} font-bold`}
              >
                {amount?.toFixed(2)} €
              </p>
            )}
